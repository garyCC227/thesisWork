...
def load_data(data_folder, batch_size, train, kwargs, val_split=0.8,target=False):
    ...
    # TO DELETE
    # _, data_idx = train_test_split(list(range(len(data))), test_size=0.1, random_state = 4)
    # new_data = Subset(data, data_idx)
    # data_loader = torch.utils.data.DataLoader(new_data, batch_size=batch_size, shuffle=True, **kwargs, drop_last = True if train else False)
    # OLD
